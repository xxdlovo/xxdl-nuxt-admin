...
<template>
  <UModal v-model:open="visible" :title="title" :dismissible="false" :ui="{
      ...
      <UForm id="sys-dept-form" ref="form" :validate="validate" :state="state" class="max-h-[min(70vh,560px)] overflow-y-auto p-1" @submit="handleSubmit">
        <div class="grid grid-cols-1 gap-x-6 gap-y-6 md:grid-cols-2">
          ...
          <UFormField name="phone" :label="$ts('module.system.department.phone')" orientation="horizontal"
            :ui="formItemUi">
            <UBaseInput v-model="state.phone" :placeholder="$ts('module.system.department.form.phone')" trailing="clear" class="w-full" />
          </UFormField>
          <UFormField name="email" :label="$ts('module.system.department.email')" orientation="horizontal"
            :ui="formItemUi">
            <UBaseInput v-model="state.email" :placeholder="$ts('module.system.department.form.email')" trailing="clear" class="w-full" />
          </UFormField>
          <UFormField name="sortOrder" :label="$ts('module.system.department.sortOrder')" orientation="horizontal"
            :ui="formItemUi">
            <UBaseInput v-model="state.sortOrder" type="number" :placeholder="$ts('module.system.department.form.sortOrder')" trailing="clear" class="w-full" />
          </UFormField>
    ...
    <template #footer>
      ...
      <UButton :label="$ts('common.confirm')" color="primary" type="submit" form="sys-dept-form" />
    </template>
  </UModal>
</template>
