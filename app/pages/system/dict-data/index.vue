<template>
  <div class="h-full flex flex-col p-3 gap-3">
    ...
    <div class="flex-shrink-0">
      <SysDictDataSearch v-model:model="searchParams" @search="getDataByPage(1, searchParams)"/>
    </div>
...
import SysDictDataSearch from './components/sys-dict-data-search.vue'
import SysDictDataOperate from "./components/sys-dict-data-operate.vue"
...
const {
  ...
  refresh,
  getDataByPage
} = usePaginatedTable<SysDictDataDto>({
  query: (params) => $trpc.sysDictData.page.query(params),
  pageSizeOptions: [10, 20, 50, 100]
})

// 表格操作 hook
const { operateType, editingData, drawerVisible, checkedRowKeys, handleAdd, handleEdit, onDeleted, onBatchDeleted, closeVisible } = useTableOperate<SysDictDataDto>({
  data,
  idKey: 'id',
  refresh
})
