<template>
  <div class="h-full flex flex-col p-3 gap-3">
    ...
    <div class="flex-shrink-0">
      <SysDictTypeSearch v-model:model="searchParams" @search="getDataByPage(1, searchParams)"/>
    </div>
    ...
    <UCard class="flex-1 min-h-0 flex flex-col overflow-hidden" :ui="{body: 'flex flex-col h-full p-0 sm:p-0' }">
      ...
      >
        <template #header>
          ...
          >
          <template #prefix>
            <span>{{ $ts('module.system.dictType.title') }}</span>
          </template>
        </TableHeaderOperation>
...
import SysDictTypeSearch from './components/sys-dict-type-search.vue'
...
// 搜索参数
const searchParams = ref<SysDictTypeQueryDTO>({})
...
const {
  ...
  refresh,
  getDataByPage
} = usePaginatedTable<SysDictTypeDto>({
  query: (params) => $trpc.sysDictType.page.query(params),
  pageSizeOptions: [10, 20, 50, 100]
})

// 表格操作 hook
const { operateType, editingData, drawerVisible, checkedRowKeys, handleAdd, handleEdit, onDeleted, onBatchDeleted, closeVisible } = useTableOperate<SysDictTypeDto>({
  data,
  idKey: 'id',
  refresh
})
...
const { selectionColumn } = useSelectionColumn<SysDictTypeDto>({
  data,
  checkedRowKeys,
...
// 定义列配置（包含选择列）
const columns = computed<TableColumn<SysDictTypeDto>[]>(() => {
  return [
    // 选择列
    selectionColumn,
    // 序号列
    ...
    {
      accessorKey: 'name',
      header: () => $ts('module.system.dictType.name')
    },
    {
      accessorKey: 'code',
      header: () => $ts('module.system.dictType.code')
    },
    useBadgeColumn<SysDictTypeDto>(
      'status',
      'module.system.dictType.dictStatus',
      USER_STATUS_CONFIG,
      1
    ),
    {
...
const handleBatchDelete = async () => {
  ...
  await $trpc.sysDictType.batchDelete.mutate(checkedRowKeys.value)
