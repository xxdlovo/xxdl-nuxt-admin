      ...
      <template #content>
        <UForm ref="form" :validateOn="['input']"  :schema="schema" :state="state" class="p-2">
          <div class=" grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4   gap-x-6 gap-y-6">
            <UFormField name="name" :label="$ts('module.system.dictType.name')" orientation="horizontal"   :ui="formItemUi">
              <UBaseInput v-model="state.name" :placeholder="$ts('module.system.dictType.form.name')" trailing="clear" class="w-full" />
            </UFormField>
            <UFormField name="code" :label="$ts('module.system.dictType.code')" orientation="horizontal"   :ui="formItemUi">
...
const emit = defineEmits<{
  search: [data: SysDictTypeQueryDTO]
}>()

const schema = SysDictTypeQuerySchema
